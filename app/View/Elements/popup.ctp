...
<!--		<div class="fl" style="position: relative;">&nbsp;&nbsp;
	...
	<?php if((SES_TYPE == 1 || SES_TYPE == 2) && defined('TWITTED') && TWITTED == 0 && !in_array(PAGE_NAME,array('onbording','profile','changepassword','email_notifications','email_reports'))){
		$osTwAccount = 'TheApplescrum';
		$tweetTexts = array(
			'#Applescrum is an Awesome Project Collaboration Tool that gives full visibility and control over your projects',
			'Organize Projects, Tasks, Documents & Meeting Minutes in one place #Applescrum'.' @'.$osTwAccount,
			'Share of your ideas, feedbacks, questions and discussions across the team #Applescrum'.' @'.$osTwAccount,
			'See what\'s in progress, what needs to be done and what\'s been accomplished #Applescrum'.' @'.$osTwAccount,
			'Receive reminders, alert for close deadlines, manage tightly not to exceed budget #Applescrum'.' @'.$osTwAccount,
			'Break-down tasks into smaller ones, share documents using google Drive & Dropbox #Applescrum'.' @'.$osTwAccount,
			'Just sit back and keep on watching the Activity even while relaxing #Applescrum'.' @'.$osTwAccount,
			'Win your customers\'confidence by keeping them informed with daily scrum #Applescrum'.' @'.$osTwAccount,
			'Keep your team on their toes by reminding them by automatic emails #Applescrum'.' @'.$osTwAccount,
			'Get instant notification on your cell and respond with your inputs in no time #Applescrum'.' @'.$osTwAccount,
			'Stay on top and get weekly usage report #Applescrum'.' @'.$osTwAccount,
			'Managing Project Effectively with project collaboration tool taming inbox #Applescrum'.' @'.$osTwAccount,
			'Get Daily Progress email from team without fail #Applescrum'.' @'.$osTwAccount,
			'#Applescrum is a Awesome project management tool for You & Your Team'.' @'.$osTwAccount
		);
