...
//print '<pre>';print_r($email_res);
			...
			if ( $num_rows < 1 ) {
				...
				send_email(INFO_MAIL, '', "Failed to save Email reply in Applescrum", $Failedmessage);
				continue;*/
			}		
...
//subject of the mail to be sent
                            ...
                            //$subject = "Re: [Applescrum]:" . $projNameInSh . ":#" . $row['case_no'] . "(" . $pj_sname . ")" . "-" . stripslashes(html_entity_decode($row['title'], ENT_QUOTES));
...
<a href='".HTTP_ROOT."' target='_blank' style='font:11px verdana;text-decoration:underline;'>Applescrum</a>
</td>
</tr>
</table>
</td>
...
You are receiving this email notification because you have subscribed to applescrum, to unsubscribe, please email with subject 'Unsubscribe' to <a href='mailto:".SUPPORT_EMAIL."'>".SUPPORT_EMAIL."</a>
</td>	  
</tr>
</table>
</td>
...
";
                ...
                if ($msg != "" && !$user_id) {
                    ...
                    if ($checkOSReg_row['total'] > 0) {
                        $userMsg = '<span>You do not have access to this Project on applescrum.</span><br /><br />';
			$userMsg1 = "";
                    } else {
                        $userMsg = '<span>This email: &quot;' . $mail_id . '&quot; is not registered with Applescrum.</span><br /><br />';
                        $userMsg1 = '<span>Note: <b>To post a reply to this task, please use your Applescrum login Email ID</b>.</span><br /><br />';
                    }
...
function write2log($why, $message, $mail_id, $subject, $date) {
    ...
    send_email(DEV_EMAIL, '', "Failed to save Email reply in Applescrum", $message);
}
